...
#
use lib ('G:\Robot\OpenHoldem\lib');
...
#
my @OH_Symbolnames = qw(br dealerchair userchair call pot balance $$pc0 $$pc1 );
...
sub cache_Symbols
{
    ...
    debug_message('cache_Symbols');
    foreach my $Symbol (@OH_Symbolnames)
    {
	$OHS{$Symbol} = gws($Symbol); 
	debug_message($Symbol, '=>', $OHS{$Symbol});
    }

    debug_message('chair ' . gws('chair'));
}
...
{    
    ...
    #    to oyur functions.
    #
    debug_message('caching');
    cache_Symbols();
    debug_message('dealerpos', $OHS{'dealerpos'});
